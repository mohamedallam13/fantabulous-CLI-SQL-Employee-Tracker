...
class Role {
    ...
    async add() {
        ...
        const [rows] = await department.getInfo().promise;
        ...
        const department_id = rows.filter(row => row.name == departmentName)[0].id;
        return { promise: this.conn.promise().query(this.addQuery, [title, salary, department_id]), message: `${title} role has been added!` }
    }
    update() {
